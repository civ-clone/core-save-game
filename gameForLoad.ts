 ...
 * uninterrupted run.
 */
export const gameForLoad = (adopted: Partial<GameSlots>): Game =>
  new Game(
    Object.fromEntries(
      Object.entries(adopted).filter(
        ([slot]) => DISPOSITIONS[slot as keyof GameSlots] !== 'state'
      )
    ) as Partial<GameSlots>
  );

export default gameForLoad;
